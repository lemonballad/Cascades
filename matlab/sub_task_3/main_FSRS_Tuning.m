...
if true
    ...
    wvibs=wvib([1 5]);disp_pna=disp([1 5]);mu_eg=6.2;
end
...
%                 tic
%                 [directb(id,iw,iv,:,:),cascade_3b(id,iw,iv,:,:),~,~,~] = FSRS_TC_Tuning_BETA(wviball_TC,fcall_TC,...
%                 3,parameters_material,parameters_laser);
%             nl=nl-1;
%                 [toc]% nl]
                tic
%                 [ratio(id,iw,iv,:,:),cas(id,iw,iv,:,:),dir(id,iw,iv,:,:)] = FSRS_TC_Tuning_KAPPA(wviball_TC,fcall_TC,...
%                 nquanta,parameters_material,parameters_laser);
                [ratio2(id,iw,iv),cas2(id,iw,iv),dir2(id,iw,iv)] = FSRS_TC_Tuning_Res(wviball_TC,fcall_TC,...
                nquanta,parameters_material,parameters_laser);
            nl=nl-1;
                [toc]% nl]
...
%                 
ratio(id,iw,iv)=prefactor(id,iw,iv)*abs(cas2(id,iw,iv))./abs(dir2(id,iw,iv))/3e10;
ratio2(id,iw,iv)=prefactor(id,iw,iv)*abs(cas2(id,iw,iv))./abs(dir2(id,iw,iv))/3e10;
% rTCb(id,iw)=prefactor(id,iw,iv)*abs(cascade_3b(id,iw,iv,1))./abs(directb(id,iw,iv,1))/3e10;
        end
    end
end
%% Plot
