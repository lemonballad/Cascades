function [ratio,cas,dir] = cascade_2dRR_Res_Time(E_vib,nquanta,ovlp,...
...
f1=complex(zeros(1,ntC,iq,'double'));
f2=f1;f3=f1;f4=f1;
...
% nt1=length(kjf)/2;
ntC=nt;
% f1=fftshift(fft(f1,ntC))/(nt*2*pi/0.82);f2=fftshift(fft(f2,ntC))/(nt*2*pi/0.82);
% f3=fftshift(fft(f3,ntC))/(nt*2*pi/0.82);f4=fftshift(fft(f4,ntC))/(nt*2*pi/0.82);
% f1=f1-f1(end);f2=f2-f2(end);f3=f3-f3(end);f4=f4-f4(end);
for j=1:ntC
for jj=1:ntC
seq1(jj,j)=-(1i)^6*(f1(j)+f2(j)+f3(j)+f4(j))*(f1(jj)+f2(jj)+f3(jj)+f4(jj));%7.9e-4*
seq2(jj,j)=-(1i)^6*(conj(f1(j))+conj(f2(j))+conj(f3(j))+conj(f4(j)))*(f1(jj)+f2(jj)+f3(jj)+f4(jj));%2.7e-3*
par1(jj,j)=-(1i)^6*(f1(jj)+f2(jj)+f3(jj)+f4(jj))...
    *(f1(j+jj)+f2(j+jj)+f3(j+jj)+f4(j+jj));%8e-4*
par2(jj,j)=-(1i)^6*(f1(jj)+f2(jj)+f3(jj)+f4(jj))...
...
% figure;contourf(w,w,cas2,50,'edgecolor','none');colormap('jet');colorbar;
% pause
...
% test=direct;
figure;
subplot(2,2,1);contour(w,w,abs(1.8841e+13/3e10*cas2),50);colorbar;colormap('jet');
subplot(2,2,2);contour(w,w,abs(dir2),50);colorbar;colormap('jet');
subplot(2,2,3);contour(w,w,abs(dir2+1.8841e+13/3e10*cas2),50);colorbar;colormap('jet');
subplot(2,2,4);contour(w,w,log10(abs(1.8841e+13/3e10*cas2)./abs(dir2)),50);colorbar;colormap('jet');