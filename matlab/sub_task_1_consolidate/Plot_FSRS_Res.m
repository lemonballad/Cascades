...
for iv=1:2
    ...
    FSRS_Res_dts(iv)=FSRS_Res_dts(1);
    ...
    gc(iv)=subplot(2,3,1+3*(iv-1));contour(t,w,abs(FSRS_Res_Cascade(:,:,iv))/maxs,50);
    ...
    gd(iv)=subplot(2,3,2+3*(iv-1));contour(t,w,abs(FSRS_Res_Direct(:,:,iv))/maxs,50);
    xlabel('\tau (fs)');%xlim(wrange);
...
%     hold on
%     contour(FSRS_Res_dws,...
%         FSRS_Res_disps,FSRS_Res_Ratio(:,:,iv),50,'edgecolor','black','LineStyle',':');
