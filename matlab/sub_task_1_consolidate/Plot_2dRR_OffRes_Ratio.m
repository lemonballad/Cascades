...
maxr=max(max(max(abs(OffRes_2dRR_Ratio))));
y=-0.04;x=0.04;
% Axes positions
gcp_t_y=0.595;gcp_b_y=0.13;
...
for iv=1:2
    maxs=max(max(max(abs(OffRes_2dRR_Cas(:,:,iv)))),...
        max(max(abs(OffRes_2dRR_Dir(:,:,iv)))));
    gc(iv)=subplot(2,3,1+3*(iv-1));contourf(OffRes_2dRR_dws,OffRes_2dRR_disps,...
        abs(OffRes_2dRR_Cas(:,:,iv))/maxs,50,'edgecolor','none');
    ...
    gd(iv)=subplot(2,3,2+3*(iv-1));contourf(OffRes_2dRR_dws,OffRes_2dRR_disps,...
        abs(OffRes_2dRR_Dir(:,:,iv))/maxs,50,'edgecolor','none');
    xlabel('(\omega_L-\omega_{eg})/2\pic (cm^{-1})');
    ylabel({'Mode Displacement \it d';'Scaling Factor'});%caxis([0 maxr])
    ...
    gr(iv)=subplot(2,3,3+3*(iv-1));contourf(OffRes_2dRR_dws,OffRes_2dRR_disps,...
