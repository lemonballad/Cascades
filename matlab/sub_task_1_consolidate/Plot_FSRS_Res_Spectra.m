...
maxs=max(max(abs(real(FSRS_Res_Full(:,:,1)))));
    subplot(1,3,1);
    contour(t,w,abs(real(FSRS_Res_Cascade(:,:,1)))/maxs,50);
    xlabel('\tau (fs)');
    ...
    subplot(1,3,2);
    contour(t,w,abs(real(FSRS_Res_Direct(:,:,1)))/maxs,50);
    xlabel('\tau (fs)');
    ylabel('\omega_2/2\pic (cm^{-1})');%caxis([0 maxr])
    ...
    subplot(1,3,3);
    contour(t,w,abs(real((FSRS_Res_Full(:,:,1))))/maxs,50);
    xlabel('\tau (fs)');
    ylabel('\omega_2/2\pic (cm^{-1})');%caxis([0 maxr])
