...
XSect_2DRR_nt=128;
XSect_2DRR_Ratio=complex(zeros(5,2,2,'double'));
for isolv=1:5
    switch isolv
        case 1
            solvent='cyclohexane';
    ...
    Ks(isolv)=alpha2;%/e0^2*w_L^4*10e8*10e20;
    Wsolvs(isolv)=wsolv;
    Rhos(isolv)=rho_Solv;
    for iv=1:2
        ...
        parameters_laser.nt=XSect_2DRR_nt;
        ...
        [ratO,~,~] = cascade_2dRR_OffRes(wviball_TC,2*nquanta,ovlp_TC,...
            parameters_laser,parameters_material);
        XSect_2DRR_Ratio(isolv,iv,1)=ratO*prefactor*3e10;
        [ratR,~,~] = cascade_2dRR_Res(wviball_TC,2*nquanta,ovlp_TC,...
            parameters_laser,parameters_material);
        XSect_2DRR_Ratio(isolv,iv,2)=ratR*prefactorres/3e10;
        
        
    end
end
save('XSect2DRR','XSect_2DRR_Ratio');
'done'

[Ks,iKs]=sort(Ks);
figure;plot(Ks,0*XSect_2DRR_Ratio(iKs,2,2)...
    +XSect_2DRR_Ratio(iKs,2,1),'b^','MarkerFaceColor',[0 0 1]);
ylabel('RATIO');hold on;%axis square
plot(Ks,0*XSect_2DRR_Ratio(iKs,1,2)...
    +XSect_2DRR_Ratio(iKs,1,1),'r^','MarkerFaceColor',[1 0 0])
set(gca,'XTick',Ks,'XTickLabel',solvents(iKs),...
    'XTickLabelRotation',45,'FontSize',14,'FontWeight','Bold');
hold off

% [Wsolvs,iWsolvs]=sort(Wsolvs);
% figure; plot(Wsolvs,0*XSect_2DRR_Ratio(iWsolvs,1,2)...
%     +XSect_2DRR_Ratio(iWsolvs,1,1),'r^','MarkerFaceColor',[1 0 0])
% hold on
% plot(Wsolvs,0*XSect_2DRR_Ratio(iWsolvs,2,2)...
%     +XSect_2DRR_Ratio(iWsolvs,2,1),'b^','MarkerFaceColor',[0 0 1])
% set(gca,'XTick',Wsolvs,'XTickLabel',solvents(iWsolvs),...
%     'XTickLabelRotation',45,'FontSize',14,'FontWeight','Bold');
% hold off

% [Rhos,iRhos]=sort(Rhos);
% figure; plot(Rhos,XSect_2DRR_Ratio(iRhos,1,2)...
%     +XSect_2DRR_Ratio(iRhos,1,1),'r^','MarkerFaceColor',[1 0 0])
% hold on
% plot(Rhos,XSect_2DRR_Ratio(iRhos,2,2)...
%     +XSect_2DRR_Ratio(iRhos,2,1),'b^','MarkerFaceColor',[0 0 1])
