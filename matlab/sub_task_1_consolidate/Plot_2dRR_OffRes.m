...
for iv=1:2
    maxs=max(max(max(abs(OffRes_2dRR_Cascade(:,:,iv)))),...
        max(max(abs(OffRes_2dRR_Direct(:,:,iv)))));
    OffRes_2dRR_dts(iv)=OffRes_2dRR_dts(1);
%     dt=dts(iv);
    w=(-1/2:dw:(1/2-dw))/OffRes_2dRR_dts(iv)/c;
    if iv==1;wrange=[-1500 1500];else wrange=[-2400 2400];end
    gc(iv)=subplot(2,3,1+3*(iv-1));contour(w,w,abs(OffRes_2dRR_Cascade(:,:,iv))/maxs,50);
    ...
    gd(iv)=subplot(2,3,2+3*(iv-1));contour(w,w,abs(OffRes_2dRR_Direct(:,:,iv))/maxs,50);
    xlabel('\omega_1/2\pic (cm^{-1})');%xlim(wrange);
    ...
    gr(iv)=subplot(2,3,3+3*(iv-1));contourf(OffRes_2dRR_dws,OffRes_2dRR_disps,log10(OffRes_2dRR_Ratio(:,:,iv)),50,'edgecolor','none');
