function [cas2,dir2] = cascade_FSRS_OffResSpec_End(wviball,nquanta,ovlp,...
...
f1SA=complex(zeros(nw,iq,'double'));
f1SB=f1SA;f2SA=f1SA;f2SB=f1SA;

[W,wDf,wDi]=ndgrid(ww,wviball(1:nquanta+1),wviball(1:nquanta+1));
wD=wDf-wDi;
DAMP=damp*ones(size(wD));DAMP(wD==0)=0;
W_t=w_rp-W;
[~,wSf,wSi]=ndgrid(ww,wsolv,wsolv);
wS=wSf-wSi;
...
f1SA=sum(f1SA,2);f2SA=sum(f2SA,2);
f1SB=sum(f1SB,2);f2SB=sum(f2SB,2);
