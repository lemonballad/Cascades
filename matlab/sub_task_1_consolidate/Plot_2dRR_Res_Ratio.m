...
for iv=1:2
    ...
    gc(iv)=subplot(2,3,1+3*(iv-1));contourf(Res_2dRR_dws,Res_2dRR_disps,...
        abs(Res_2dRR_Cas(:,:,iv))/maxs,50,'edgecolor','none');
    ...
    gd(iv)=subplot(2,3,2+3*(iv-1));contourf(Res_2dRR_dws,Res_2dRR_disps,...
        abs(Res_2dRR_Dir(:,:,iv))/maxs,50,'edgecolor','none');
    xlabel('(\omega_L-\omega_{eg})/2\pic (cm^{-1})');
    ylabel({'Mode Displacement \it d';'Scaling Factor'});%caxis([0 maxr])
    ...
    gr(iv)=subplot(2,3,3+3*(iv-1));contourf(Res_2dRR_dws,Res_2dRR_disps,...
