...
FSRS_Res_Cascade=FSRS_Res_Cascade;
FSRS_Res_Direct=-FSRS_Res_Direct;
FSRS_Res_Full=FSRS_Res_Cascade+FSRS_Res_Direct;
y=-0.04;x=0.04;
% Axes positions
gcp_t_y=0.605;gcp_b_y=0.15;
...
w=(-1/2:dw:(1/2-dw))/FSRS_Res_dts/c;w=w-w(1);
t=0:FSRS_Res_dts:(FSRS_Res_nt-1)*FSRS_Res_dts;
for iv=1:2
    maxs=max(max(abs(real(FSRS_Res_Full(:,:,iv)))));
    gc(iv)=subplot(2,3,1+3*(iv-1));
    contour(t,w,abs(real(FSRS_Res_Cascade(:,:,iv)))/maxs,50);
    xlabel('\tau (fs)');
    ...
    gd(iv)=subplot(2,3,2+3*(iv-1));
    contour(t,w,abs(real(FSRS_Res_Direct(:,:,iv)))/maxs,50);
    xlabel('\tau (fs)');
    ylabel('\omega_2/2\pic (cm^{-1})');%caxis([0 maxr])
    ...
    gr(iv)=subplot(2,3,3+3*(iv-1));
    contour(t,w,abs(real((FSRS_Res_Full(:,:,iv))))/maxs,50);
    xlabel('\tau (fs)');
    ylabel('\omega_2/2\pic (cm^{-1})');%caxis([0 maxr])
