...
maxr=max(max(max(abs(FSRS_OffRes_Ratio))));
y=-0.04;x=0.04;
% Axes positions
...
for iv=1:2
    maxs=max(max(max(abs(FSRS_OffRes_Cascade(:,:,iv)))),...
        max(max(abs(FSRS_OffRes_Direct(:,:,iv)))));
    FSRS_OffRes_dts(iv)=FSRS_OffRes_dts(1);
    w=(-1/2:dw:(1/2-dw))/FSRS_OffRes_dts(iv)/c;w=w-w(1);
    t=0:FSRS_OffRes_dts(iv):(FSRS_OffRes_nt-1)*FSRS_OffRes_dts(iv);
    if iv==1;wrange=[-1500 1500];else wrange=[-2400 2400];end
    gc(iv)=subplot(2,3,1+3*(iv-1));contour(t,w,-imag(FSRS_OffRes_Cascade(:,:,iv))/maxs,50);
    xlabel('\tau (fs)');%xlim(wrange);
    ylabel('\omega/2\pic (cm^{-1})');%ylim(wrange);'Xtick',[1e-4 3e-4 5e-4],'XtickLabel',{'1.0e-4','3.0e-4','5.0e-4'},
    ...
    gd(iv)=subplot(2,3,2+3*(iv-1));contour(t,w,-real(FSRS_OffRes_Direct(:,:,iv))/maxs,50);
    xlabel('\tau (fs)');%xlim(wrange);
    ylabel('\omega/2\pic (cm^{-1})');%ylim(wrange);
    ...
    gr(iv)=subplot(2,3,3+3*(iv-1));contourf(FSRS_OffRes_dws,FSRS_OffRes_disps,log10(FSRS_OffRes_Ratio(:,:,iv)),50,'edgecolor','none');
    xlabel('(\omega_AP-\omega_{eg})/2\pic (cm^{-1})');
    ylabel({'Scaling Factor of';'Mode Displacement \it d'});%caxis([0 maxr])
