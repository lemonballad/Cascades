...
% Signal frequency
w_t=weg;
FSRS_CH2Cl2_Vib_wsolv=0:50:2000;
nw=length(FSRS_CH2Cl2_Vib_wsolv);
FSRS_CH2Cl2_Vib_Ratio=complex(zeros(nw,2,'double'));
% matlabpool open 10
for iv=1:2
    if iv==1
...
LAMBDA_rp=40;
%
FSRS_CH2Cl2_Vib_nt=128;

    wvib=wvibs(1);
    dt=FSRS_CH2Cl2_Vib_dts;(iv);
    for iw=1:length(FSRS_CH2Cl2_Vib_wsolv)
        w_ap=weg;
        w_rp=w_ap;
        wsolv=FSRS_CH2Cl2_Vib_wsolv(iw);
        tic
            Disp=disp_pna;
            ...
            parameters_laser.nt=FSRS_CH2Cl2_Vib_nt;
            ...
            FSRS_CH2Cl2_Vib_Ratio(iw,iv)=prefactor*3e10*rat;
...
save('FSRSCH2Cl2Vib','FSRS_CH2Cl2_Vib_Ratio','FSRS_CH2Cl2_Vib_wsolv','rcas','rdir','rcas2','rdir2');
'done'


figure;plot(FSRS_CH2Cl2_Vib_wsolv,abs(rcas(:,1)+...
    rcas2(:,1))./abs(rdir(:,1)),'r-','Linewidth',2);hold on
plot(FSRS_CH2Cl2_Vib_wsolv,abs(rcas(:,2)+...
