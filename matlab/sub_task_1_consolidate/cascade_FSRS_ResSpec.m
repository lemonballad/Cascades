function [cas2,dir2] = cascade_FSRS_ResSpec(wviball,nquanta,ovlp,...
...
[W,tau_1D,wDf,wDi]=ndgrid(ww,tau1,wviball,wviball);
...
D0=exp((-1i*wD*r2w-DAMP).*tau_1D).*(2*LAMBDA_ap)./(wD.^2+LAMBDA_ap^2);
% J(w_t-w_RP)
J=1./(W_t-w_rp-wD+1i*(gamma_vib-LAMBDA_rp)).*exp((1i*(w_rp-W_t)-LAMBDA_rp)*tau2*r2w);
