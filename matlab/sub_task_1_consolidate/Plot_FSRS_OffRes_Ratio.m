...
clear all
load FSRSOfftest
FSRS_OffRes_Cas=rcas;
FSRS_OffRes_Dir=rdir;
gcf=figure;
...
for iv=1:2
    maxs=max(max(max(abs(FSRS_OffRes_Cas(:,:,iv)))),...
        max(max(abs(FSRS_OffRes_Dir(:,:,iv)))));
    gc(iv)=subplot(2,3,1+3*(iv-1));contourf(FSRS_OffRes_dws,FSRS_OffRes_disps,...
        abs(FSRS_OffRes_Cas(:,:,iv))/maxs,50,'edgecolor','none');
    ...
    gd(iv)=subplot(2,3,2+3*(iv-1));contourf(FSRS_OffRes_dws,FSRS_OffRes_disps,...
        abs(FSRS_OffRes_Dir(:,:,iv))/maxs,50,'edgecolor','none');
    xlabel('(\omega_{AP}-\omega_{eg})/2\pic (cm^{-1})');
    ylabel({'Mode Displacement \it d';'Scaling Factor'});%caxis([0 maxr])
    ...
    gr(iv)=subplot(2,3,3+3*(iv-1));contourf(FSRS_OffRes_dws,FSRS_OffRes_disps,...
