...
Lc1=1i./(-wvib-w+1i*gamma_vib);%2*gamma_vib./((wvib+w).^2+gamma_vib^2);%1./(wvib+w+1i*gamma_vib);%exp((-1i*wD*r2w-damp).*tau_1D);
Lc2=1i./(-wvib-w+1i*gamma_vib);%2*gamma_vib./((wvib+w).^2+gamma_vib^2);%1./(wvib+w+1i*gamma_vib);%exp((-1i*wD*r2w-damp).*tau_2D);
Lc0=1i./(-wvib-w+1i*gamma_vib);%2*gamma_vib./((wvib+w).^2+gamma_vib^2);%1./(wvib+w+1i*gamma_vib);%exp((-1i*wC*r2w-damp).*tau0);
Lc0C=1i./(-wvib+w+1i*gamma_vib);%2*gamma_vib./((wvib-w).^2+gamma_vib^2);%1./(wvib-w+1i*gamma_vib);%exp((-1i*wC*r2w-damp).*tau0);
LcP=1i./(W+1i*gamma_vib);%2*gamma_vib./((wvib-w).^2+gamma_vib^2);%1./(wvib-w+1i*gamma_vib);%exp((-1i*wC*r2w-damp).*tau0);
        ...
        for k=1:iq
            ...
            for l=1:iq
                ...
                if k~=m
                    ...
                    f1C(m)=f1C(m)+boltz_pop(m)*ovlp(n,m)*ovlp(n,k)*ovlp(l,k)*ovlp(l,m)...
                        *Lp(n,m)*Lc0C(k,m)*Lp(l,m);
                    f2C(m)=f2C(m)+boltz_pop(m)*ovlp(n,m)*ovlp(n,k)*ovlp(l,m)*ovlp(l,k)...
                ...
                if k~=n
                    ...
                    f3C(m)=f3C(m)+boltz_pop(m)*ovlp(n,m)*ovlp(k,m)*ovlp(k,l)*ovlp(n,l)...
                        *Lp(n,m)*Lc0C(n,k)*Lp(n,l);
                    f4C(m)=f4C(m)+boltz_pop(m)*ovlp(n,m)*ovlp(k,m)*ovlp(n,l)*ovlp(k,l)...
...
% figure
% subplot(2,2,1);plot(ww,abs(f1P),'b-');
% subplot(2,2,2);plot(ww,abs(f2P),'b-');
% subplot(2,2,3);plot(ww,abs(f3P),'b-');
% subplot(2,2,4);plot(ww,abs(f4P),'b-');
% figure;plot(ww,abs(f1P+f2P+f3P+f4P),'b-');
...
wr=(w(end)-ww(1));
par1=(conv((f1PA+f2PA+f3PA+f4PA),(f1PB+f2PB+f3PB+f4PB),'same'))*(f1PA+f2PA+f3PA+f4PA).'*wr/2/pi/c/20;
par2=par1;

seq=seq1+seq2;

par=par1+par2;%par=par*10;
[~,iomega]=min(abs(ww-wvib));
cascade=seq*pi/dt+par(iomega,iomega);

cas=abs(cascade);
dir=abs(direct);
ratio=cas/dir;

% figure;contour(ww,ww,abs(par),50);colorbar;colormap('jet');
% % subplot(1,2,1);contour(ww,ww,abs(par),50);colorbar;colormap('jet');
% % subplot(1,2,2);contour(w,w,abs(direct),50);colorbar;colormap('jet');
% pause