...
*1/(wL-weg-(n-m)*wvib+1i*gam_eg)*1/(wL-weg-(l-m)*wvib+1i*gam_eg)*1/(wL-weg-(v-m)*wvib+1i*gam_eg)...
.*exp(-1i*(k-m)*wvib*r2w*t1(kj)-km*gam_vib*t1(kj)).*exp(-1i*(u-m)*wvib*r2w*t2(ii)-um*gam_vib*t2(ii));
...
*1/(-wL+weg-(m-n)*wvib+1i*gam_eg)*1/(-wL+weg-(m-l)*wvib+1i*gam_eg)*1/(wL-weg-(v-u)*wvib+1i*gam_eg)...
.*exp(-1i*(m-k)*wvib*r2w*t1(kj)-km*gam_vib*t1(kj)).*exp(-1i*(m-u)*wvib*r2w*t2(ii)-um*gam_vib*t2(ii));
end  
%
mk=1;
uk=1;
...
*1/(wL-weg-(n-m)*wvib+1i*gam_eg)*1/(-wL+weg-(k-l)*wvib+1i*gam_eg)*1/(wL-weg-(v-u)*wvib+1i*gam_eg)...
.*exp(-1i*(k-m)*wvib*r2w*t1(kj)-mk*gam_vib*t1(kj)).*exp(-1i*(k-u)*wvib*r2w*t2(ii)-uk*gam_vib*t2(ii));
r4(ii,kj)=r4(ii,kj)+p(m)*ovlp(n,m)*ovlp(n,k)*ovlp(l,m)*ovlp(l,u)*ovlp(v,u)*ovlp(v,k)...
*1/(-wL+weg-(m-n)*wvib+1i*gam_eg)*1/(wL-weg-(l-k)*wvib+1i*gam_eg)*1/(wL-weg-(v-k)*wvib+1i*gam_eg)...
.*exp(-1i*(m-k)*wvib*r2w*t1(kj)-mk*gam_vib*t1(kj)).*exp(-1i*(u-k)*wvib*r2w*t2(ii)-uk*gam_vib*t2(ii));
end
%
end
end
