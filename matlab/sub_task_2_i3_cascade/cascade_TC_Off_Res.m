function [ratio,cas,dir] = cascade_TC_Off_Res(E_vib,nquanta,ovlp,...
...
f1_SolvP=complex(zeros(iq,1,'double'));f2_SolvP=f1_SolvP;

[tau_1D,tau_2D,wDf,wDi]=ndgrid(tauD,tauD,E_vib,E_vib);
[tau0,wCf,wCi]=ndgrid(tauC,E_vib,E_vib);
[wSi,wSf]=ndgrid(wsolv,wsolv);
wD=wDf-wDi;
wC=wCf-wCi;
wS=wSf-wSi;
wS0=wS.*(diag(ones(1,iq),1)+diag(ones(1,iq),-1));
...
LcP=1./(wvib-wS0-w+2i*gamma_vib);%exp((-1i*wS*r2w-damp).*tau0);
...
for m=1:1
    ...
    parfor n=1:iq
        if abs(n-m)==1
            %
            f1_Solv(n)=f1_Solv(n)+boltz_pop_Solv(m)*LcS(n,m);
            f2_Solv(n)=f2_Solv(n)+boltz_pop_Solv(m)*LcS(m,n);
            f1_SolvC(n)=f1_SolvC(n)+boltz_pop_Solv(m)*LcSC(n,m);
            f2_SolvC(n)=f2_SolvC(n)+boltz_pop_Solv(m)*LcSC(m,n);
            f1_SolvP(n)=f1_SolvP(n)+boltz_pop_Solv(m)*LcP(n,m);
            f2_SolvP(n)=f2_SolvP(n)+boltz_pop_Solv(m)*LcP(m,n);
            %
        end
        ...
        for k=1:iq
            ...
            for l=1:iq
                ...
                if k~=m
                    ...
                    f1_SolC(n)=f1_SolC(n)+boltz_pop(m)*ovlp(n,m)*ovlp(n,k)*ovlp(l,k)*ovlp(l,m)...
                        *Lp(n,m)*Lc0C(k,m)*Lp(l,m);
                    f2_SolC(n)=f2_SolC(n)+boltz_pop(m)*ovlp(n,m)*ovlp(n,k)*ovlp(l,m)*ovlp(l,k)...
                        *Lm(m,n)*Lc0C(m,k)*Lp(l,k);
                    f1_SolP(n)=f1_SolP(n)+boltz_pop(m)*ovlp(n,m)*ovlp(n,k)*ovlp(l,k)*ovlp(l,m)...
                        *Lp(n,m)*LcP(k,m)*Lp(l,m);
                    f2_SolP(n)=f2_SolP(n)+boltz_pop(m)*ovlp(n,m)*ovlp(n,k)*ovlp(l,m)*ovlp(l,k)...
                ...
                if k~=n
                    ...
                    f3_SolC(n)=f3_SolC(n)+boltz_pop(m)*ovlp(n,m)*ovlp(k,m)*ovlp(k,l)*ovlp(n,l)...
                        *Lp(n,m)*Lc0C(n,k)*Lp(n,l);
                    f4_SolC(n)=f4_SolC(n)+boltz_pop(m)*ovlp(n,m)*ovlp(k,m)*ovlp(n,l)*ovlp(k,l)...
                        *Lm(m,n)*Lc0C(k,n)*Lp(k,l);
                    f3_SolP(n)=f3_SolP(n)+boltz_pop(m)*ovlp(n,m)*ovlp(k,m)*ovlp(k,l)*ovlp(n,l)...
                        *Lp(n,m)*LcP(n,k)*Lp(n,l);
                    f4_SolP(n)=f4_SolP(n)+boltz_pop(m)*ovlp(n,m)*ovlp(k,m)*ovlp(n,l)*ovlp(k,l)...
...
%                                     *Lp(v,l);
    ...
    f1_SolvP=sum(f1_SolvP);f2_SolvP=sum(f2_SolvP);
...
seq2=(f1_Sol+f2_Sol+f3_Sol+f4_Sol+f1_SolC+f2_SolC+f3_SolC+f4_SolC)...
    *(f1_Solv+f2_Solv);
par1=2*(f1_SolvP+f2_SolvP)...
    *(f1_Sol+f2_Sol+f3_Sol+f4_Sol);
par2=2*(f1_SolP+f2_SolP+f3_SolP+f4_SolP)...
    *(f1_Solv+f2_Solv);


% for t_1=1:ntC
% for t_2=1:ntC
% seq1=-1i*(f1_Sol+f2_Sol)*(f1_Sol+f2_Sol);
% seq2=-1i*(conj(f1_Sol)+conj(f2_Sol))*(conj(f1_Solv)+conj(f2_Solv));
% par1=-1i*(f1_Sol+f2_Sol+f3_Sol+f4_Sol);
% par2=-1i*(f1_Sol+f2_Sol+f3_Sol+f4_Sol);
% end
% end
...
cas=abs(cascade);
dir=abs(direct);%dir2(iomega,iomega);
ratio=cas/dir;
