...
f2=f1;f3=f1;f4=f1;ff1=f1;ff2=f1;ff3=f1;ff4=f1;

% [tau_1,tau_2,wi,wf]=ndgrid(taud,taud,E_vib,E_vib);
[taudd,wwi,wwf]=ndgrid(tau,E_vib,E_vib);
% w=wf-wi;
ww=wwf-wwi;
[tau_1,tau_2]=meshgrid(taud);
Lp=1./(w_L-weg-w+1i*gamma_eg);
Lm=1./(-w_L+weg-w+1i*gamma_eg);
% Lc1=exp((-1i*w*r2w-damp).*tau_1);
% Lc2=exp((-1i*w*r2w-damp).*tau_2);
Lc0=exp((-1i*ww*r2w-damp).*taudd);
...
%     ff1=sum(ff1,3);ff2=sum(ff2,3);ff3=sum(ff3,3);ff4=sum(ff4,3);
%     figure;subplot(2,2,1);plot(tau,f1,'b-',tau,ff1,'r--');
%     subplot(2,2,2);plot(tau,f2,'b-',tau,ff2,'r--');
%     subplot(2,2,3);plot(tau,f3,'b-',tau,ff3,'r--');
%     subplot(2,2,4);plot(tau,f4,'b-',tau,ff4,'r--');
