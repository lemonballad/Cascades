...
%     disps=[0.05 (0.1:0.1:5)];nd=length(disps);
    dws=-3000:100:3000;nw=length(dws);
    disps=(0.1:0.1:5);nd=length(disps);
   for iw=1:nw
        for id=1:nd
            ...
            % Compute E(3):E(5) prefactor
            prefactor(id,iw,iv)=prefactor_3_5(l,C,mu_eg,n_w_t,w_L+wvib/2);
...
%             [toc]% rat cas dir]
            tic
            [rat,cas,dir] = cascade_TC_Res(wviball_TC,nquanta,ovlp_TC,...
                parameters_laser,parameters_material,wvib);
            ratioG(id,iw,iv)=rat*prefactor(id,iw,iv)/3e10;rcasG(id,iw,iv)=cas;rdirG(id,iw,iv)=dir;
            [rat2,cas2,dir2] = cascade_TC_ResCopy(wviball_TC,nquanta,ovlp_TC,...
                parameters_laser,parameters_material,wvib);
